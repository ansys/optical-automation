...
class TestMaterialAPI:
    ...
    def setup_class(self):
        ...
        self.results_file = os.path.join(self.local_path, "input", "results_materials.json")
        reference_file = os.path.join(self.local_path, "input", "reference_results_materials.json")
        self.clean_results(self)  # no idea why but you have to pass there self

        scdm_exe = os.path.join(scdm_install_dir, "SpaceClaim.exe")
        scdm_script_path = os.path.join(self.local_path, "input", "run_material_api.py")
