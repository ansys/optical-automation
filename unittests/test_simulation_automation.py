...
class TestSimAutomation:
    ...
    def setup_class(self):
        ...
        self.results_file = os.path.join(self.local_path, "input", "results_sim_automation.json")
        reference_file = os.path.join(self.local_path, "input", "reference_results_sim_automation.json")
        self.clean_results(self)  # no idea why but you have to pass there self

        scdm_exe = os.path.join(scdm_install_dir, "SpaceClaim.exe")
        scdm_script_path = os.path.join(self.local_path, "input", "run_simulation_automation.py")
    ...
    def test_02_camera_spectra(self):
        ...
        test_passed = (res_red == ref_red and
                       res_green == ref_green and
                       res_blue == ref_blue)
        assert test_passed
