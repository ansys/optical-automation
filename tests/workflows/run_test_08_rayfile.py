...
def check_converted_rayfile(rayfile_path, file_type):
    ...
    return reference == input_ray_file
...
def check_speos_sim(rayfile_path):
    ...
    command = [executable, r"-C", r"-S", r"0000", sim_path]
    subprocess.call(command)
    result = False
    html_file = glob.glob(os.path.join(work_directory, "*,html"))
    for file in html_file:
        result = result and os.path.exists(file)
...
def main():
    ...
    convert = RayfileConverter(test_file)
    convert.zemax_to_speos()
    results_dict[sdf_ray_sim] = check_speos_sim(sim_path, os.path.splitext(test_file)[0].lower() + ".ray")
    # test08
    test_file = os.path.join(work_directory, "test_08_dat.dat")
    shutil.copyfile(ray_file, test_file)
    convert = RayfileConverter(test_file)
    convert.zemax_to_speos()
    results_dict[dat_ray_sim] = check_speos_sim(sim_path, os.path.splitext(test_file)[0].lower() + ".ray")
