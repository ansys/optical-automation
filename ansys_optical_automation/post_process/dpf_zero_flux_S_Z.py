...
class DpfRayfile_zero(DataProcessingFramework):
    """
    ...
    def load_content(self):
        ...
        if rayfile_type == "ray":
            ...
            #count ray number without 0 flux
            self.__real_ray_number = self.__ray_numb
            for ray_idx in range(self.__ray_numb):
                ...
                if e <  0:
                    ...
                    self.__real_ray_number = self.__real_ray_number - 1
                else:
            ...
            if ray_format_type == 0:
                ...
                raise TypeError(msg)
            #count ray number without 0 flux
            real_ray_number = self.__ray_numb
            for ray_idx in range(self.__ray_numb):
                x = struct.unpack("f", self.dpf_instance.read(4))[0]
                y = struct.unpack("f", self.dpf_instance.read(4))[0]
                z = struct.unpack("f", self.dpf_instance.read(4))[0]
