...
if current_sel.Count == 1:
    ...
    if hodStatus==False:
            ApplicationHelper.ReportWarning("The HOD feature selected is not up to date, please verify the simulation inputs")
    ...
    if eyebox_number > 1:
        ...
        hoaSim.Eyebox.EyeboxConfigPositionDirectionTypeIndex=hodObject.EyeboxConfigPositionDirectionTypeIndex #Finding Position direction
   ...
   #Windshield definition
    ...
    while str(hodObject.Projectors[k].ProjectorType) != "PGU":
        ...
        if hodObject.Projectors[k].MultiConfig == True:
            ProjectorMultiID=k
            MultiTicked=True
        k += 1
        NbProjectorHOD += 1 
            ...
            if MultiTicked ==True:
                ...
                for curve in TiltAxisCurves:
                    if curve.GetName() == TiltAxisToSearch:
                        ...
                        TiltAxisFound = True
                        break
                if TiltAxisFound == False:
                    Display = MessageBox.Show("Tilt axis can't be found, please select it manually and set the rotation values")
                    ApplicationHelper.ReportWarning("Tilt axis can't be found, please select it manually and set the rotation values")
    ...
    if MultiTicked == True :
        Display = MessageBox.Show("HOA simulation created, do not forget to add warping, report and windshield outer face if needed. Please set the mirrors rotation angle values")
        ApplicationHelper.ReportInformation("HOA simulation created, do not forget to add warping, report and windshield outer face if needed. Please set the mirrors rotation angle values")
    #Warping, report and windshield outer face doesn't exist in HOD feature so can't be retrieve
    else:            
        Display = MessageBox.Show("HOA simulation created, do not forget to add warping, report and windshield outer face if needed")
        ApplicationHelper.ReportInformation("HOA simulation created, do not forget to add warping, report and windshield outer face if needed")
