...
checkerSize = (
    62.5  # (mm); the physical, measured side-length of each individual checkerboard box in the calibration pattern
)
...
pixelSize = 0.024  # (mm); physical sensor pixel size (need to convert calibrated distortion into angles for speos)
...
def buildinitialcameramatrix(efl, rX, rY):
    ...
    cameraMat[0, 2] = (
        0.5 * rX
    )  # image center coordinates X (units=pixels); this should be set to 1/2 the sensor X resolution
    cameraMat[1, 2] = (
        0.5 * rY
    )  # image center coordinates Y (units=pixels); this should be set to 1/2 the sensor Y resolution
    return cameraMat
    ...
    if useFisheyeCalibration:
        ...
        ret, cameraMatrix, distortion, r_vecs, t_vecs = cv2.calibrateCamera(
            ...
            flags=(
                ...
                + cv2.CALIB_FIX_PRINCIPAL_POINT  # initial matrix guess
                + cv2.CALIB_RATIONAL_MODEL  # don't allow cx/cy as variable
                + cv2.CALIB_ZERO_TANGENT_DIST  # use 6 k-terms
            ),
        )  # only symmetrical distortion
...
def convertdistortiontoangles(cameraMatrix, resolution, distortion, useFisheyeCalibration):
    ...
    xyzObj = np.zeros(
        (nSamp, 1, 3), dtype=np.float32
    )  # note: theres a bug (?) in cv2.fisheye.projectPoints, where it needs an extra dimension in the input array
...
def main():
    ...
    twoDPoints, threeDPoints, imagesAnalyzed, resolution = readcalibrationimages(
        images, checkerDims, checkerSize, showCalImages
    )
    ...
    ret, cameraMatrix, distortion, r_vecs, t_vecs = cameracalibration(
        initialCameraMat, resolution, twoDPoints, threeDPoints, useFisheyeCalibration
    )
