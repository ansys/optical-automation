...
class OSModeConverter:
    ...
    def angle2rm(self, theta_x, theta_y, theta_z, order):
        ...
        if order == 0:
            ...
            rotation_matrix[1, 0] = math.cos(theta_x) * math.sin(theta_z) + math.sin(theta_x) * math.sin(
                theta_y
            ) * math.cos(theta_z)
            rotation_matrix[1, 1] = math.cos(theta_x) * math.cos(theta_z) - math.sin(theta_x) * math.sin(
                theta_y
            ) * math.sin(theta_z)
            rotation_matrix[1, 2] = -math.sin(theta_x) * math.cos(theta_y)

            rotation_matrix[2, 0] = math.sin(theta_x) * math.sin(theta_z) - math.cos(theta_x) * math.sin(
                theta_y
            ) * math.cos(theta_z)
            rotation_matrix[2, 1] = math.sin(theta_x) * math.cos(theta_z) + math.cos(theta_x) * math.sin(
                theta_y
            ) * math.sin(theta_z)
            ...
            rotation_matrix[0, 0] = math.cos(theta_y) * math.cos(theta_z)
            rotation_matrix[0, 1] = -math.sin(theta_z) * math.cos(theta_x) + math.sin(theta_x) * math.sin(
                theta_y
            ) * math.cos(theta_z)
            rotation_matrix[0, 2] = math.sin(theta_x) * math.sin(theta_z) + math.cos(theta_x) * math.sin(
                theta_y
            ) * math.cos(theta_z)
            ...
            rotation_matrix[1, 2] = -math.cos(theta_z) * math.sin(theta_x) + math.cos(theta_x) * math.sin(
                theta_y
            ) * math.sin(theta_z)
    ...
    def test_reverse(self, rotation_matrix, toSurf, thesystem, last_rotation_matrix, ref_rotation_matrix):
        """
        function to test whether tilt 180 degree needed
        it uses chief ray tracing result in sequential system to determine
        the direction after refraction into the media following to Surf
        if the surface orientation is opposite to the chief ray direction
        then the surface tilt about local Y 180 degree
        Parameters
        ----------
        rotation_matrix: list
        ...
        re = raytrace.SingleRayNormUnpol(
        ...
        index_n = re[8]
        raytrace.Close()
        vi = np.array([[index_l], [index_m], [index_n]])
    ...
    def convert(self, list_rotation_matrix, list_position, reverseflag_ob1):
        ...
        # ref_rotation_matrix= RM
        # pitfall: this will pass the address of RM to ref_rotation_matrix, which is not correct
        # for intermediate variable, use list instead
        ...
        for i in range(1, len(list_rotation_matrix)):
            ...
            rotation_matrix = self.test_reverse(
                rotation_matrix, 2 * i, se_sys, last_rotation_matrix, ref_rotation_matrix
            )
